...
<body>
    ...
    <div class="container-fluid">
        ...
        <a class="navbar-brand" href="index.php">VIEW USERS AND ADMINS</a>
        </div>
...
<div style="width:100px; height:100px;"></div>
    ...
    <div class="row">
        <div class="col-sm-5">
            <h4><font color="white">ADMINS</font></h4>
    ...
    <tbody>
        <?php
        include '../db/db.php';
          $query="SELECT * FROM logins WHERE who='admin'";
          $run_query=mysqli_query($conn,$query);
          while($sql=mysqli_fetch_assoc($run_query)){
      echo '<tr>
        <td>'.$sql['name'].'</td>
        <td>'.$sql['usename'].'</td>
        <td>'.$sql['password'].'</td><td><a href="viewaduse.php?del='.$sql['id'].'" class="btn btn-danger btn-xs">DELETE</a></td>';
        ...
        <div class="col-sm-5">
           <h4><font color="white">USERS</font></h4>
          <table class="table table-hover">
    <thead>
      <tr>
        <th>Name</th>
    ...
    <tbody>
        <?php
         include '../db/db.php';
          $query="SELECT * FROM logins WHERE who='user'";
          $run_query=mysqli_query($conn,$query);
          while($sql=mysqli_fetch_assoc($run_query)){
      echo '<tr>
        <td>'.$sql['name'].'</td>
